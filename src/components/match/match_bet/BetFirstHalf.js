...
class BetFirstHalf extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    const betFirstHalf = nextProps.betFirstHalf;
    if (betFirstHalf) {
      const bet_scores = betFirstHalf.bet_content.split('-');
      const home_score = bet_scores[0];
      const away_score = bet_scores[1];
      const bet_amount = betFirstHalf.bet_amount;
      return {
        ...prevState,
      ...
      return {
        ...
        // disableBet:
        //   nextProps.match_status >= 0 && nextProps.match_status <= 45 ? false : true,
      };
    }
  }
  ...
  render() {
    const { home, away, betFirstHalf } = this.props;
